...
def argument_parsing() :
    ...
    parser = argparse.ArgumentParser(description='SubpathwayCascade.Final')

    parser.add_argument('--SOURCE_GENE_SET', type=str, help='source gene set name', default='CGC_Hallmark_Tier1')
    parser.add_argument('--TARGET_GENE_SET', type=str, help='target gene set name', default='Prosigna')
    ...
    parser.add_argument('--EVIDENCE_CUTOFF', type=int, help='TFTG evidence cutoff', default=3)
...
def read_source_target_gene_sets() :
    ...
    tier1_CGC = pd.read_csv("COSMIC_GRCh38_COSMICv96_Tier1.tsv", sep="\t")
    tier1_CGC_hallmark = tier1_CGC[tier1_CGC["Hallmark"].notna()]
    tier1_CGC_hallmark_genes = set(tier1_CGC_hallmark["Gene Symbol"])
            
    hallmarks = pd.read_csv("Cancer_Gene_Census_Hallmarks_Of_Cancer.tsv", sep="\t", encoding="unicode_escape")
    ...
    pam50_genes = set(pd.read_csv(os.path.join(os.getcwd(), "GeneSignatures", "pam50.tsv"), sep="\t")["probe"])
    endopredict_genes = set(pd.read_csv(os.path.join(os.getcwd(), "GeneSignatures", "sig.endopredict.tsv"), sep="\t")["symbol"])
    oncotypedx_genes = set(pd.read_csv(os.path.join(os.getcwd(), "GeneSignatures", "sig.oncotypedx.tsv"), sep="\t")["symbol"])
    ...
    PAM50_SYMBOLS = extract_updated_symbols(pd.read_csv(os.path.join(os.getcwd(), "GeneSignatures", "sig.pam50.HUGO.tsv"), sep="\t"), pam50_genes)
    ENDOPREDICT_SYMBOLS = extract_updated_symbols(pd.read_csv(os.path.join(os.getcwd(), "GeneSignatures", "sig.endopredict.HUGO.tsv"), sep="\t"), endopredict_genes)
    ONCOTYPEDX_SYMBOLS = extract_updated_symbols(pd.read_csv(os.path.join(os.getcwd(), "GeneSignatures", "sig.oncotypedx.HUGO.tsv"), sep="\t"), oncotypedx_genes)
...
def read_external_TFTG_reference(evidence_cutoff=2) :
    ...
    TFTG_path = os.path.join(os.getcwd(), "TFTG")
...
def read_pathway_reference() :
    ...
    ORGANISM = "hsa"
    KEGG_path = os.path.join(os.getcwd(), "KEGG", ORGANISM)
...
def generate_TFTG_bridge(external_TFTGs=None) :
    ...
    #if not external_TFTGs is None : CASCADE_TFTGS = pathway_TFTGs | external_TFTGs
...
if __name__ == "__main__" : 
    ...
    SUBPATHWAY_FOLDER_PATH = os.path.join(os.getcwd(), "SubpathwayCascade.Final", ".".join(HYPERPARAMETERS))
